...
import com.example.wallpapermanager.DatabaseListView.databaseView;
    ...
    ActivityResultLauncher<Intent> getFolderUri = registerForActivityResult(new ActivityResultContracts.StartActivityForResult(),
            new ActivityResultCallback<ActivityResult>() {
                ...
                public void onActivityResult(ActivityResult result) {
                    ...
                    ExceptionLogHelper errorDb = new ExceptionLogHelper(MainActivity.this);
                    if (result.getResultCode() == Activity.RESULT_OK) {
                        dbThread = new Thread(
                                () -> {
                                    ...
                                    if (directory != null) {
                                        ...
                                        for (DocumentFile f : files) {
                                            if (f.isFile()) {
                                                if (f.getType().matches("(^)image(.*)")) {
                                                    ...
                                                    //imageDbHelper.insert(imageModel);
                                                    forSave.add(imageModel);
                                                    forSave.get(fileCount - 1).setLast(false);
                                        ...
                                        Log.e("DIRECTORY PICK", "directory is null");
                                        DateTimeFormatter dtf = DateTimeFormatter.ofPattern("dd/MM/yyyy HH:mm:ss");
                                        LocalDateTime now = LocalDateTime.now();
                                        ExceptionModel em = new ExceptionModel("DIRECTORY PICK", "cannot find first image", dtf.format(now), "LockReceiver");
                                        errorDb.insert(em);
                                    }
                        ...
                        Log.e("DIRECTORY PICK", "Result Value is "+result.getResultCode());
                        DateTimeFormatter dtf = DateTimeFormatter.ofPattern("dd/MM/yyyy HH:mm:ss");
                        LocalDateTime now = LocalDateTime.now();
                        ExceptionModel em = new ExceptionModel("DIRECTORY PICK", "cannot find first image", dtf.format(now), "LockReceiver");
                        errorDb.insert(em);
